...
Did you forget to decorate your state class `{state.__name__}` with @stateclass?"""
  ...
  def restore_state_from_session(self, session_id: str):
    state_session.restore(session_id, self._states)

  def save_state_to_session(self, session_id: str):
    state_session.save(session_id, self._states)
